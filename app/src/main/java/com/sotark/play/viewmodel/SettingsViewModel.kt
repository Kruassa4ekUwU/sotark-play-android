...
class SettingsViewModel @Inject constructor(
    ...
    fun setLanguage(lang: AppLanguage) {
        ...
        when (lang) {
            AppLanguage.UKRAINIAN -> {
                ukrainianTapCount++; israelTapCount = 0
                if (ukrainianTapCount >= 10) { settings.unlockEasterEgg(); ukrainianTapCount = 0 }
            }
            AppLanguage.HEBREW -> {
                israelTapCount++; ukrainianTapCount = 0
                if (israelTapCount >= 10) { settings.unlockIsraelEasterEgg(); israelTapCount = 0 }
            }
    ...
    fun toggleUkrainianTheme() = settings.setUkrainianTheme(settings.secretTheme.value != SecretTheme.UKRAINIAN)
